 ...
 * Actif en dev, ou si `localStorage.setItem("wa:debug", "1")` côté navigateur,
 * ou si l'URL contient `?wa-debug=1`. Permet de diagnostiquer en production
...
const isWaDebugEnabled = (): boolean => {
  try {
    ...
    if (window.localStorage?.getItem("wa:debug") === "1") return true;
...
export const buildWaUrl = (
  ...
  // Toujours utiliser wa.me — lien universel qui fonctionne sur mobile (ouvre l'app)
  // et sur desktop (redirige vers WhatsApp Web). Évite l'embed `web.whatsapp.com/send`
  // qui refuse les requêtes hors contexte d'app officielle (ERR_BLOCKED_BY_RESPONSE).
  return number
...
/**
 * Ouvre WhatsApp hors de l'iframe de prévisualisation.
 * Certains environnements embarqués réécrivent les liens `wa.me` vers
 * `api.whatsapp.com/send`, ce qui finit bloqué par les headers de frame.
 * En ouvrant explicitement une nouvelle fenêtre dans le geste utilisateur,
 * on évite cette navigation dans l'iframe.
 */
export const openWaUrl = (
  event: { preventDefault: () => void } & { defaultPrevented?: boolean },
  url: string | null,
  source: string,
  meta: Record<string, unknown> = {},
): void => {
  trackWaClick(source, meta);

  if (!url || typeof window === "undefined") return;

  // Détection iframe (aperçu Lovable, intégrations…)
  let inIframe = false;
  try {
    inIframe = window.self !== window.top;
  } catch {
    inIframe = true; // cross-origin → on est forcément dans une iframe
  }

  // Hors iframe : on laisse le <a target="_blank"> natif faire le travail.
  // Cela évite les blocages de popup et garantit l'ouverture en nouvel onglet.
  if (!inIframe) return;

  // Dans une iframe : on doit empêcher la navigation par défaut (qui chargerait
  // wa.me DANS l'iframe → bloqué par X-Frame-Options) et forcer une ouverture
  // hors iframe.
  event.preventDefault();

  // 1) Tente une popup (souvent bloquée dans les iframes sandboxées)
  ...
  // 2) Tente une navigation au niveau du parent (top-level) — fonctionne
  //    quand l'iframe partage l'origine ou autorise la navigation top.
  try {
    ...
    /* cross-origin top → on tombe sur le fallback ci-dessous */
  }

  // 3) Dernier recours : ouvrir l'URL dans l'onglet courant.
  //    On évite window.location.assign à l'intérieur de l'iframe (bloqué),
  //    on utilise donc une ancre temporaire avec target="_blank".
