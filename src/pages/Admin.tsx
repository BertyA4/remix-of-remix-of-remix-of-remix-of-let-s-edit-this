...
const Admin = () => {
  ...
  const [accountForm, setAccountForm] = useState({ email: "", newPassword: "", confirmPassword: "", currentPassword: "" });
  const [savingAccount, setSavingAccount] = useState(false);

  useEffect(() => { if (user?.email) setAccountForm((f) => ({ ...f, email: user.email ?? "" })); }, [user?.email]);

  const updateAccount = async () => {
    if (!user?.email) return;
    const newEmail = accountForm.email.trim().toLowerCase();
    const emailChanged = newEmail && newEmail !== user.email.toLowerCase();
    const wantsPwdChange = !!accountForm.newPassword;

    if (!emailChanged && !wantsPwdChange) return toast.error("Aucune modification");
    if (emailChanged && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newEmail)) return toast.error("Email invalide");
    if (wantsPwdChange) {
      if (accountForm.newPassword.length < 12) return toast.error("Mot de passe : 12 caractères min.");
      if (!/[A-Z]/.test(accountForm.newPassword) || !/[a-z]/.test(accountForm.newPassword) || !/\d/.test(accountForm.newPassword) || !/[^A-Za-z0-9]/.test(accountForm.newPassword))
        return toast.error("Inclure majuscule, minuscule, chiffre et symbole");
      if (accountForm.newPassword !== accountForm.confirmPassword) return toast.error("Les mots de passe ne correspondent pas");
    }
    if (!accountForm.currentPassword) return toast.error("Saisis ton mot de passe actuel pour confirmer");

    setSavingAccount(true);
    try {
      // Re-authenticate
      const { error: reauthErr } = await supabase.auth.signInWithPassword({
        email: user.email, password: accountForm.currentPassword,
      });
      if (reauthErr) throw new Error("Mot de passe actuel incorrect");

      const updates: { email?: string; password?: string } = {};
      if (emailChanged) updates.email = newEmail;
      if (wantsPwdChange) updates.password = accountForm.newPassword;

      const { error } = await supabase.auth.updateUser(updates);
      if (error) throw error;

      await log("update_admin_account", "user", user.id, {
        email_changed: emailChanged, password_changed: wantsPwdChange,
      });
      setAccountForm({ email: emailChanged ? newEmail : user.email, newPassword: "", confirmPassword: "", currentPassword: "" });
      toast.success(emailChanged ? "Vérifie ta boîte mail pour confirmer le nouvel email" : "Mot de passe mis à jour");
    } catch (e: any) {
  ...
  const updateContentByKey = async (key: string, value: string) => {
    ...
    try {
      ...
      localStorage.setItem("lovable-content-updated", `${key}:${Date.now()}`);
    } catch { /* noop */ }
      ...
      <div className="container max-w-6xl">
        ...
        <Tabs defaultValue="quotes" className="space-y-6">
          ...
          <TabsContent value="account" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>Modifier mes identifiants</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 max-w-xl">
                ...
                <div>
                  <Label htmlFor="acc-email">Nouvel email</Label>
                  <Input id="acc-email" type="email" autoComplete="email" value={accountForm.email}
                    onChange={(e) => setAccountForm({ ...accountForm, email: e.target.value })} maxLength={255} />
                  <p className="mt-1 text-[11px] text-muted-foreground">Un email de confirmation sera envoyé à la nouvelle adresse.</p>
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    <Label htmlFor="acc-pwd">Nouveau mot de passe</Label>
                    <Input id="acc-pwd" type="password" autoComplete="new-password" value={accountForm.newPassword}
                      onChange={(e) => setAccountForm({ ...accountForm, newPassword: e.target.value })} maxLength={72} placeholder="Laisser vide pour ne pas changer" />
                  </div>
                  <div>
                    <Label htmlFor="acc-pwd2">Confirmer</Label>
                    <Input id="acc-pwd2" type="password" autoComplete="new-password" value={accountForm.confirmPassword}
                      onChange={(e) => setAccountForm({ ...accountForm, confirmPassword: e.target.value })} maxLength={72} />
                  </div>
                </div>
                <p className="text-[11px] text-muted-foreground">Min. 12 caractères, avec majuscule, minuscule, chiffre et symbole.</p>
                <div>
                  <Label htmlFor="acc-current">Mot de passe actuel (confirmation)</Label>
                  <Input id="acc-current" type="password" autoComplete="current-password" value={accountForm.currentPassword}
                    onChange={(e) => setAccountForm({ ...accountForm, currentPassword: e.target.value })} maxLength={72} />
                </div>
                <Button onClick={updateAccount} disabled={savingAccount} className="btn-primary">
