...
type SectionKey =
  | "brand" | "hero" | "trust" | "about" | "services" | "showcase"
...
const VisualEditor = () => {
  ...
  // Scroll the preview iframe to the selected section when changed via sidebar
  useEffect(() => {
    const f = iframeRef.current;
    if (!f) return;
    const target = SECTIONS.find((s) => s.key === section)?.anchor;
    if (!target) return;
    try {
