...
import { buildWaUrl, openWaUrl, trackWaClick } from "@/lib/whatsapp";
...
const ServiceDetail = () => {
  ...
  useEffect(() => {
    if (!slug) return;
    try {
      const raw = localStorage.getItem("piham:saved-services");
  ...
  const handleSave = () => {
    ...
    try {
      ...
      localStorage.setItem("piham:saved-services", JSON.stringify(next));
      setSaved(next.includes(slug));
      toast.success(next.includes(slug) ? "Service enregistré" : "Service retiré");
    } catch {
        ...
        <div className="mt-6 grid grid-cols-4 grid-rows-2 gap-2 rounded-2xl overflow-hidden h-[280px] md:h-[480px]">
          ...
          {gallery.slice(1, 5).map((img, i) => (
            ...
            >
              ...
              {i === 3 && (
                <div className="absolute bottom-3 right-3 bg-white text-primary text-xs font-semibold px-3 py-2 rounded-lg shadow-card flex items-center gap-2">
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <aside className="lg:col-span-5">
            ...
            <a
              ...
              onClick={(e) => openWaUrl(e, buildWaUrl("22899500054", `Bonjour, je souhaite discuter du service ${service?.title ?? "PIHAM"}.`) ?? "https://wa.me/?text=Bonjour", "service_detail", { service: service?.slug })}
