  ...
  <footer id="footer" className="relative bg-primary text-white py-14">
    <div className="container">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          <Link to="/" className="flex items-center gap-3 w-fit">
            <span className={`h-10 w-10 rounded-xl flex items-center justify-center overflow-hidden ${logoUrl ? "bg-transparent" : "bg-white"}`}>
              {logoUrl ? (
        ...
        <div>
          <div className="text-xs uppercase tracking-[0.25em] text-white/60 font-semibold">{t("footer.navigation")}</div>
          <ul className="mt-4 space-y-2 text-sm">
            <li><Link to="/#services" className="hover:text-[hsl(var(--accent-glow))] transition-colors">{t("nav.services")}</Link></li>
            <li><Link to="/#showcase" className="hover:text-[hsl(var(--accent-glow))] transition-colors">{t("nav.showcase")}</Link></li>
            <li><Link to="/#about" className="hover:text-[hsl(var(--accent-glow))] transition-colors">{t("nav.about")}</Link></li>
            <li><Link to="/#contact" className="hover:text-[hsl(var(--accent-glow))] transition-colors">{t("nav.contact")}</Link></li>
            {pages.map((p) => (
        ...
        <div>
          <div className="text-xs uppercase tracking-[0.25em] text-white/60 font-semibold">{t("footer.contactUs")}</div>
      ...
      <div className="mt-12 pt-6 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-white/60">
        <div>© {new Date().getFullYear()} {brandName} {brandTag}. {t("footer.rights")}</div>
        <div>{credit}</div>
      </div>
    </div>
  </footer>
