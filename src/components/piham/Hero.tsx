import { motion, type Variants, useMotionValue, useSpring, useTransform, useInView, animate } from "framer-motion";
...
export const Hero = () => {
  ...
  if (stats.length === 0) {
    stats = [
      { v: (content["hero.stat1_value"] ?? "150+").trim(), l: (content["hero.stat1_label"] ?? "Projets livrés").trim() },
      { v: (content["hero.stat2_value"] ?? "12 ans").trim(), l: (content["hero.stat2_label"] ?? "d'expertise terrain").trim() },
      { v: (content["hero.stat3_value"] ?? "98%").trim(), l: (content["hero.stat3_label"] ?? "Clients fidélisés").trim() },
    ...
    <HeroSection>
      ...
      <div className="container relative py-12 md:py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-end">
          ...
          >
            <TiltCard>
            <div className="relative aspect-[4/5] rounded-3xl overflow-hidden shadow-elevated group">
              <img
                src={heroImage}
                alt={heroImageAlt}
                data-editable-key="hero.image_url"
              ...
              <div data-editable-key="hero.card_rating" className="absolute top-4 right-4 glass-strong rounded-full px-3 py-1.5 text-[10px] uppercase tracking-[0.2em] font-semibold text-foreground">
                {(content["hero.card_rating"] ?? "★ 4.92 / 5").trim()}
              </div>
              ...
              <div className="absolute inset-x-4 bottom-4 flex items-end justify-between gap-3">
                <div className="text-white">
                  <div className="text-[10px] uppercase tracking-[0.25em] text-white/70 font-semibold">
                    {(content["hero.card_eyebrow"] ?? "Chantier · Lomé").trim()}
                  </div>
                  <div className="font-display text-lg font-semibold mt-0.5">
                    {(content["hero.card_title"] ?? "Pôle BTP — 2026").trim()}
                  </div>
                </div>
                <motion.div
                  whileHover={{ rotate: 45, scale: 1.1 }}
                  transition={{ type: "spring", stiffness: 250, damping: 18 }}
                  className="h-10 w-10 rounded-full bg-white text-background flex items-center justify-center shrink-0 shadow-card"
                >
                  →
                </motion.div>
              </div>
        ...
        >
          ...
          <div className="lg:col-span-6 flex flex-wrap items-center gap-3 lg:justify-end">
            ...
            <MagneticButton
              ...
              onClick={(e) => openWaUrl(e, buildWaUrl("22899500054", "Bonjour, je souhaite échanger avec PIHAM Info Services.") ?? "https://wa.me/?text=Bonjour", "hero")}
        ...
        <div className="flex gap-12 animate-marquee whitespace-nowrap">
          {[...Array(2)].map((_, k) => (
            <div key={k} className="flex items-center gap-12 shrink-0">
              {["Génie civil", "Réhabilitation patrimoine", "Fibre optique", "Datacenter", "Routes & VRD", "Bâtiments industriels", "Réseau télécom", "Fourniture matériaux", "Maîtrise d'œuvre"].map((w) => (
                <div key={w + k} className="flex items-center gap-12 text-foreground">
                  <span className="font-display text-2xl md:text-3xl tracking-tight font-medium">{w}</span>
